...
class Page extends MY_Controller {
	...
	public function topup(){
		$this->smarty->assign('site_title','TOPUP - '.$this->settings['site_title']);
		$this->smarty->assign('message',null);
		$this->smarty->assign('prod_id','2');
		if($this->session->userdata('logged')==true){

			$prod = $this->db->get_where('product',array('prod_id'=>2))->row();
	...
	public function ppob(){
		$this->smarty->assign('site_title','PPOB - '.$this->settings['site_title']);
		$this->smarty->assign('message',null);
		$this->smarty->assign('prod_id','3');
		if($this->session->userdata('logged')==true){

			$prod = $this->db->get_where('product',array('prod_id'=>3))->row();
			redirect("$prod->prod_name");
		}else{
